...
Let $X$ and $Y$ be smooth manifolds and $\pi : X \to Y$ a smooth surjection.
An open set $V \subset N$ is called \emph{evenly covered} if the connected components of $\pi^{-1}[V]$ are each diffeomorphic to $V$ under $\pi$.
...
This is surjective local diffeomorphism.
...
\]
But clearly $(2\pi - \varepsilon,2\pi)$ is not diffeomorphic to $V$ under $\pi$, it is not even surjective.
...
\begin{theorem}
\label{thm:lift}
\textup{\cite[Thm~3.23]{Warner1983}}\\
Suppose that $\pi : X \to Y$ is a covering map and $Z$ is a connected and simply connected space.
Then for any map $\alpha: Z \to Y$, point $z \in Z$, and point $x \in \pi^{-1}[\alpha(z)]$ there is a map $\tilde{\alpha} : Z \to X$ with $\alpha = \pi \circ \tilde{\alpha}$ and $\tilde{\alpha}(z) = x$.
We say that $\tilde{\alpha}$ is the lift of $\alpha$ through $x$.
\end{theorem}
\begin{proof}[Sketch of proof.]
The proof of uniqueness only requires connectedness of $Z$.
Suppose there were two lifts and consider the set of points of $Z$ on which they agree.
By definition they agree at $z$.
Then one uses continuity to argue that it is closed, and the covering property to argue that it is open.
Hence it is all of $Z$.

The proof of existence is usually carried out in three stages.
In the first stage we take $Z$ to be a closed interval.
The image is compact, so can be covered by finitely many evenly covered open sets of $Y$.
Starting with the evenly covered set containing $\alpha(z)$, we know that there is a diffeomorphic neighbourhood $U$ of $x$.
Then $(\pi|_U)^{-1} \circ \alpha$ defines a local lift of $\alpha$.
Repeating this along the path gives $\tilde{\alpha}$ and proves the result in this case.

In the second step, we take $Z$ to be a product of closed intervals.
Now we use the same idea as the first stage. 
The difference is that there is no clear order in which to proceed, so this is finicky.

In the third and final stage, we let $Z$ be any simply connected space.
For any point $z'\in Z$, take a path $p$ from $z$ to $z'$.
From the first stage, we know that $\alpha \circ p$ has a lift $\tilde{p}$.
Define $\tilde{\alpha}(z') = \tilde{p}(z')$.
It remains to show that this is independent of the chosen path $p$.
But if there is another path, then because $Z$ is simply connected, there is a homotopy between the two paths.
But this is just a fancy name for a map $h : [0,1]\times[0,1] \to Z$ with $h(0,\cdot)$ equal to the first path, $h(1,\cdot)$ equal to the second path, $h(\cdot,0) = z$, and $h(\cdot,1) = z'$.
Lifting $\alpha \circ h$ shows that the lifts of the two paths have the same endpoint in $X$.
\end{proof}

For any space, there is essentially a unique simply connected covering space.

\begin{theorem}
Let $X$ be a connected smooth manifold.
Then there exists a smooth manifold $\tilde{X}$, called the \emph{universal cover} of $X$, and smooth covering map $\pi : \tilde{X} \to X$.
\end{theorem}
\begin{proof}[Sketch of proof.]
Remarkably the proof is constructive.
Fix a base point of $X$.
The universal cover $\tilde{X}$ is the set of paths originating at the base point, up to homotopy equivalence.
The covering map sends a path to its endpoint.
For any path $\gamma$ take a simply connected open neighbourhood $V$ of its endpoint $y = \pi(\gamma) = \gamma(1)$.
...
The reason for the epitaph `universal' is that if $\pi : X \to Y$ is a covering map, and $\pi_Y : \tilde{Y} \to Y$ is the universal cover, then the lifting property tells us that $\pi_Y$ lifts to a covering map $\pi_X$ of $X$, with $\pi_Y = \pi \circ \pi_X$.

\begin{corollary}
The universal cover is unique in the following sense:
If $X,X'$ are simply connected and $\pi : X \to Y, \pi': X'\to Y$ are covering maps, then for any points $y \in Y$, $x \in \pi^{-1}[y]$, and $x' \in \pi^{-1}[y]$ there is a diffeomorphism from $X$ to $X'$ that takes $x$ to $x'$
\end{corollary}

We can apply this corollary to the universal cover itself.
Given two points $x,x' \in \pi^{-1}[Y]$ there is an automorphism of $\tilde{X}$ taking $x$ to $x'$.
